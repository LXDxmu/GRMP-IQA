...
class VPTVisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.get_embeddings:
            ...
            # np.save("/home/xmu/NR-IQA-IJCAJ/MetaIQA-master/heap_map/npy_50/baseline_cls_token.npy", global_embedding[0].detach().cpu())
            ...
            if len(self.out_indices) == 1:
                ...
                # if x[:,1:101].shape[0] != 234:
                    # print("x[1:101]", x[:,1:101].shape)
                    # np.save("/home/xmu/NR-IQA-IJCAJ/MetaIQA-master/heap_map/npy_50/vpt_soft.npy", x[:,1:101].detach().cpu())
        ...
        for i in range(self.num_layers):
            if i == 0:
                ...
                deep_prompt_emb = self.prompt_dropout(
                    self.prompt_proj(self.deep_prompt_embeddings[i - 1]).expand(B, -1, -1)).permute(1, 0, 2)
    ...
    def forward_reverse_deep_prompt(self, embedding_output, features, H, W, out_last=False):
        ...
        for i in range(self.num_layers):
            if i == 0:
                ...
                deep_prompt_emb = self.prompt_dropout(
                    self.prompt_proj(self.deep_prompt_embeddings[i - deep_num_no - 1]).expand(B, -1, -1)).permute(1, 0,
                                                                                                                  2)
                hidden_states = torch.cat((
                    hidden_states[:1, :, :],
                    deep_prompt_emb,
...
class VPTCLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            vision_heads = vision_width // 64
            self.visual = VPTVisionTransformer(
                input_resolution=image_resolution,
                patch_size=vision_patch_size,
                width=vision_width,
                layers=vision_layers,
